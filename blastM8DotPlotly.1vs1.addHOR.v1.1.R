...
option_list <- list(
  make_option(c("-i","--input"), type="character", default=NULL,
              help="m8 file from blast program [default %default]",
              dest="input_filename"),
  ...
  make_option(c("-r", "--reference-ids"), type="character", default=NULL,
              ...
              dest="refIDs"),
  make_option(c("--refLen"), type="numeric", default=NULL,
			  help="length of the ref",
			  dest="refLen"),
  make_option(c("--qryLen"), type="numeric", default=NULL,
              help="length of the qry",
			  dest="qryLen"),
  make_option(c("--refHOR"), type="character", default=NULL,
              help="ref HOR bed",
              dest="refHOR"),
  make_option(c("--qryHOR"), type="character", default=NULL,
              help="qry HOR bed",
              dest="qryHOR"),
  make_option(c("--col"), type="character", default=NULL,
			  help="color table",
			  dest="col")
)
...
alignments = alignments[,-c(5,6,11,12)]
...
#colnames(alignments) = c("refStart","refEnd","queryStart","queryEnd","lenAlnRef","lenAlnQuery","percentID","refID","queryID")
colnames(alignments) = c("queryID", "refID", "percentID", "lenAlnQuery", "queryStart", "queryEnd", "refStart", "refEnd")
...
base = min(opt$qryLen, opt$refLen)*0.05
#base = 1e6
...
if (opt$similarity) {
  gp = ggplot(alignments) +
    geom_point(
	  ...
	  mapping = aes(x = refStart2, y = queryStart2, color = percentID),
      size = 0.009
    ) +
    geom_point(
      #mapping = aes(x = refEnd2, y = queryEnd2, color = percentIDmean),
      mapping = aes(x = refEnd2, y = queryEnd2, color = percentID),
      size = 0.009
    ) +
    geom_segment(
      aes(
	...
	scale_x_continuous(limits = c(0-base,opt$refLen), expand = c(0,0)) +
    theme_bw() +
    ...
    #                              color = "grey60",
    #                              size = .1) }} +
	scale_y_continuous(limits = c(0-base,opt$qryLen), expand = c(0,0)) +
    scale_color_distiller(palette = "Spectral", limits = c(95,100)) +
  ...
  gp = ggplot(alignments) +
    ...
    #scale_x_continuous(breaks = cumsum(as.numeric(chromMax)),
    #                   labels = levels(alignments$refID)) +
    scale_x_continuous(limits = c(0,opt$refLen), expand = c(0,0)) +
    theme_bw() +
    theme(text = element_text(size = 8)) +
    theme(
      panel.grid.major.y = element_blank(),
    ...
    #                              color = "grey60",
    #                              size = .1) }} +
	scale_y_continuous(limits = c(0,opt$qryLen), expand = c(0,0)) +
...
refHOR = read.table(opt$refHOR, h=F, sep="\t")
qryHOR = read.table(opt$qryHOR, h=F, sep="\t")

refcol = t(data.frame(strsplit(refHOR$V6, ',')))
rownames(refcol) = NULL
refcol1 = apply(refcol, 1, function(x){rgb(as.numeric(x[1]), as.numeric(x[2]), as.numeric(x[3]), maxColorValue = 255)})
refYMAX = apply(refHOR, 1, function(x){if(x[5] == '+'){1}else{-1}})
refHOR1 = cbind(refHOR, refcol1, refYMAX)

qrycol = t(data.frame(strsplit(qryHOR$V6, ',')))
rownames(qrycol) = NULL
qrycol1 = apply(qrycol, 1, function(x){rgb(as.numeric(x[1]), as.numeric(x[2]), as.numeric(x[3]), maxColorValue = 255)})
qryYMAX = apply(qryHOR, 1, function(x){if(x[5] == '+'){1}else{-1}})
qryHOR1 = cbind(qryHOR, qrycol1, qryYMAX)

refHOR2 = cbind(refHOR1[,c(1,2,3)], rep(0, nrow(refHOR1)), rep(0-base, nrow(refHOR1)), refHOR1[,4])
colnames(refHOR2) = c('chr', 'xmin', 'xmax', 'ymin', 'ymax', 'type')
qryHOR2 = cbind(qryHOR1[,1], rep(0, nrow(qryHOR1)), rep(0-base, nrow(qryHOR1)), qryHOR1[,c(2,3)], qryHOR1[,4])
colnames(qryHOR2) = c('chr', 'xmin', 'xmax', 'ymin', 'ymax', 'type')
HOR = data.frame(rbind(refHOR2, qryHOR2))
#HOR$type = factor(as.character(HOR$type), levels=unique(HOR$type))

#library(tidyverse)
library(ggnewscale)

colTab = read.table(opt$col, h=F, sep="\t", comment.char='')
cols = result <- setNames(as.character(colTab$V2), as.character(colTab$V1))
print(cols)
gp = gp + new_scale_colour() + geom_rect(data = HOR, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, fill = type, color = type)) + scale_color_manual(values = cols) + scale_fill_manual(values = cols)
#gp = gp + geom_rect(data = refHOR1, aes(xmin = V2, xmax = V3, ymin = 0, ymax = base, fill = V4, color = V4))
#gp = gp + geom_rect(data = qryHOR1, aes(xmin = 0, xmax = base, ymin = V2, ymax = V3, fill = V4, color = V4))
#gp = gp + geom_rect(aes(data = refHOR1, xmin = refHOR1$V2, xmax = refHOR1$V3, ymin = rep(0, nrow(refHOR1)), ymax = refHOR1$refYMAX*base, fill = refHOR1$V4, color = refHOR1$V4))
#gp = gp + geom_rect(aes(data = qryHOR1, xmin = rep(0, nrow(qryHOR1)), xmax = qryHOR1$qryYMAX*base, ymin = qryHOR1$V2, ymax = qryHOR1$V3, fill = qryHOR1$V4, color = qryHOR1$V4))
