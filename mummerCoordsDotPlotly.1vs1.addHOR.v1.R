...
option_list <- list(
  ...
  make_option(c("--qryLen"), type="numeric", default=NULL,
			  ...
			  dest="qryLen"),
  make_option(c("--HOR"), type="character", default=NULL,
              help="HOR bed",
              dest="HOR")
)
...
if (opt$similarity) {
  gp = ggplot(alignments) +
	...
	scale_x_continuous(limits = c(1,opt$refLen), expand = c(0,0)) +
    theme_bw() +
    ...
    #                              color = "grey60",
    #                              size = .1) }} +
	scale_y_continuous(limits = c(1,opt$qryLen), expand = c(0,0)) +
...
HOR = read.table(opt$HOR, h=F, sep="\t")

HORcol = t(data.frame(strsplit(HOR$V8, ',')))
rownames(HORcol) = NULL
refcol1 = apply(HORcol, 1, function(x){rgb(as.numeric(x[1]), as.numeric(x[2]), as.numeric(x[3]), maxColorValue = 255)})
